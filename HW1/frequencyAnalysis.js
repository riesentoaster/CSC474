...
export const base = 'A'.charCodeAt(0)

export const shiftText = (text, amount) =>
  text
    .split('')
    .map((e, i) => (e.charCodeAt(0) - base + amount) % 26 + base)
...
export const getScore = (text) => Array(26).fill(0).map((_, i) => shiftText(text, i)).map((e, i) => [i, frequencyAnalysis(e).score]).sort((a, b) => a[1] - b[1])

export const getMostProbableShiftCount = (text) => getScore(text)[0][0]
